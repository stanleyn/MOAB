#purpose: implement the test to identtify the important bridge nodes on the 3 modality dataset

library('igraph')
library('doParallel')
library('foreach')

#load dataset
source('TCGA_sarc/Process_TCGA_Sarcoma.R')

Raw=t(AllX)
print(dim(Raw))

PointLab=nodelab
source('MOAB_main/CompCorTest.R')

#tSNE
Tolo=readRDS('TCGA_sarc/embeddings/tSNE')
TGraph=readRDS('TCGA_sarc/Layout_Graphs/tSNE')
graph=graph.adjacency(TGraph,mode='undirected',weighted=TRUE)
tSNE=CompCorTest(Raw,graph)[,1]
print('tSNE')
print(sum(tSNE))

#umap
Tolo=readRDS('TCGA_sarc/embeddings/UMAP')
TGraph=readRDS('TCGA_sarc/Layout_Graphs/umap')
graph=graph.adjacency(TGraph,mode='undirected',weighted=TRUE)
umap=CompCorTest(Raw,graph)[,1]
print('umap')
print(sum(umap))

#largeVis
Tolo=readRDS('TCGA_sarc/embeddings/LargeVis')
TGraph=readRDS('TCGA_sarc/Layout_Graphs/LV')
graph=graph.adjacency(TGraph,mode='undirected',weighted=TRUE)
LV=CompCorTest(Raw,graph)[,1]
print('LV')
print(sum(LV))

#TriMap
Tolo=readRDS('TCGA_sarc/embeddings/TriMap')
TGraph=readRDS('TCGA_sarc/Layout_Graphs/trimap')
graph=graph.adjacency(TGraph,mode='undirected',weighted=TRUE)
trimap=CompCorTest(Raw,graph)[,1]
print('trimap')
print(sum(trimap))

#PCA
Tolo=readRDS('TCGA_sarc/embeddings/PrComp')
TGraph=readRDS('TCGA_sarc/Layout_Graphs/PCA')
graph=graph.adjacency(TGraph,mode='undirected',weighted=TRUE)
PCA=CompCorTest(Raw,graph)[,1]
print('pca')
print(sum(PCA))

#LAMP
Tolo=readRDS('TCGA_sarc/embeddings/Lamp')
TGraph=readRDS('TCGA_sarc/Layout_Graphs/Lamp')
graph=graph.adjacency(TGraph,mode='undirected',weighted=TRUE)
Lamp=CompCorTest(Raw,graph)[,1]
print(sum(Lamp))
print('lamp')

Quality_Scores=c(sum(tSNE),sum(umap),sum(LV),sum(trimap),sum(PCA),sum(Lamp))
names(Quality_Scores)=c('tSNE','umap','largeVis','trimap','pca','lamp')
