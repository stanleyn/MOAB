GlobalKL_Btw=function(Orig,EmbedList,PointLab){
library('doParallel')
library('foreach')
...
#get label for node i
labi=PointLab[toGet[i]]
allLabs=PointLab[toGet]
relInds=which(allLabs!=labi)

OtherEmbed=DistEmbed[i,relInds]

E=OtherEmbed/max(OtherEmbed)
EmbedCDF=ecdf(E)(E)

#EmbedCDF=E
OtherOrig=DistOrig[i,relInds]

O=OtherOrig/max(OtherOrig)
OrigCDF=ecdf(O)(O)
